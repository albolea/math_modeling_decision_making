...
+ The maximum profit is obtained by: $x_1$ = `r round(nlp$solution[1], 2)`, $x_2$ = `r round(nlp$solution[2], 2)`, and $\pi$ = \$`r -(round(nlp$objective, 2))`
...
+ Mean = `r round(mean(res_w_dual[,1]-res_w[,1]),2)`  
+ Standard deviation =`r round(sd(res_w_dual[,1]-res_w[,1]),2)`  
...
+ A server is expected to be busy `r round(rho*100,2)`% of times  
...
tab_q3%>% 
  ...
  kable_styling(bootstrap_options = c("striped", "bordered")) %>% 
  add_footnote(
    paste0("Probability all servers are busy is ",
           round(B_erlang(s, lambda/mu)*100, 1),
           "%"),
    notation = "symbol") %>% 
  add_footnote(
    paste0("Probability customers will wait in the queue ", 
...
tab_q3 %>%
  ...
  geom_line() +
  labs(title = "Most likely to have one customer in the system", 
...
+ On an average, there are `r mean(tab_q3$L)` people in the system and `r mean(tab_q3$Lq)` person in the queue  
...
$\sum_{i=0}^{1}P_i = P_0 + P_1 =$ `r sum(sum(tab_q3$Pn[1:2]))*100`%  
...
tab_q3 %>%
  ...
  geom_line() +
  labs(title = "We see an important redicyion in queue time going from 2 to 4 servers",
       y = "Minutes in the queue") +
  scale_x_continuous("Servers", breaks = seq(0, n, by = 1)) +
  theme_classic()

```


+ Adding two more servers can reduce the wait time in the queue to `r round(line_4_servers,2)` minutes  
...
+ The probability that at least one of the call servers will be able to work on email inquiries in when there are none or one customer in the system which is:  
$\sum_{i=0}^{2}P_i = P_0 + P_1 =$ `r sum(sum(tab_q3$Pn[1:3]))*100`%  
...
tab_q3 %>%
  ggplot(mapping = aes(x = Servers, y = `Wq (minutes)`)) +
  geom_line() +
  labs(title = "Keeping 4 servers continues the best solution",
       y = "Minutes in the queue") +
  scale_x_continuous("Servers", breaks = seq(0, n, by = 1)) +
  theme_classic()
...
```{r echo = FALSE}
# Add column names
colnames(tab_q3) <- c("Servers", "L", "Lq", "W (minutes)", "Wq (minutes)", 
...
+ The project's mean NPV is:$ `r npv_mean`
+ The project's mean STD is:$ `r npv_sd`